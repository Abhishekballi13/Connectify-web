...
- fix security bug - can i send message to a person who is not my friend ? - auth in web sockets
   - if im not friend is should not be able to chat with that person.(done)
  - non logged in user should not be able to chat(done)
  - build feature show green dot if online ,(done)
  - also this one [last seen - 2 hours ago].
